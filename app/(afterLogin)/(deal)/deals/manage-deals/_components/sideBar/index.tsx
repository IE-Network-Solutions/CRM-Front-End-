...
function DealSideBar({ open, onClose }: SideBarProps) {
  ...
  footer={
    <div className="flex justify-center items-center gap-4">
      <Button type="primary" className="font-md bg-[#4080f0] text-white h-10" onClick={() => {}}>
        Create Deal
      </Button>
      <Button type="default" className="font-md border-[#4080f0] text-[#4080f0] h-10" onClick={() => {}}>
        Cancel
      </Button>
    </div>
  }
>
  <Form layout="vertical" onValuesChange={() => {}} className="w-full">
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <Form.Item name="opening" label="Opening" rules={[{ required: true }]}>
        <DatePicker className="w-full" placeholder="Start Date" />
      </Form.Item>
      <Form.Item name="closing" label="Closing" rules={[{ required: true }]}>
