...
import { Lead, CreateLeadRequest, UpdateLeadStageRequest } from "@/store/server/features/leads/interface"
...
interface LeadActions {
  ...
  updateMultipleLeadStages: (leadIds: number[], stageId: number) => Promise<boolean>
...
export const useLeadActions = create<LeadActions>((set, get) => ({
  ...
  deleteMultipleLeads: async (leadIds: number[]) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || "Failed to delete leads"
      set({ loading: false, error: errorMessage })
      return false
    }
  },
  ...
  updateMultipleLeadStages: async (leadIds: number[], stageId: number) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || "Failed to update lead stages"
      set({ loading: false, error: errorMessage })
      return false
    }
  },
  ...
  searchLeads: async (searchTerm: string) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || "Failed to search leads"
      set({ loading: false, error: errorMessage })
      return []
    }
  },
  ...
  filterLeads: async (filters: any) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || "Failed to filter leads"
      set({ loading: false, error: errorMessage })
      return []
    }
  },
